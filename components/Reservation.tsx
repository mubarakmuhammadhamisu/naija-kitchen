...
const INFO_ITEMS = [
  {
    ...
    title: "Opening Hours",
    lines: ["Mon – Sun", "10:00 AM – 11:00 PM", "Open Everyday"],
  },
  {
    icon: MapPin,
  ...
  {
    icon: Phone,
    iconColor: "text-naija-amber",
    iconBg: "bg-naija-amber/15",
    title: "Call Us",
    lines: ["+234 901 234 5678", "+234 803 456 7890"],
  },
];
...
const SOCIAL_LINKS = [
  ...
  },
  {
    label: "TikTok",
    href: "#",
    color: "bg-black hover:bg-naija-card-2 border border-naija-border",
    icon: (
      <svg viewBox="0 0 24 24" fill="currentColor" className="w-4 h-4">
...
export default function Reservation() {
      ...
      (entries) => {
        entries.forEach((entry) => {
          if (entry.isIntersecting) {
            entry.target
              .querySelectorAll<HTMLElement>(".res-animate")
              .forEach((el, i) => {
                setTimeout(() => {
                  el.style.opacity = "1";
  ...
  return (
    <section
      ...
      style={{
        backgroundImage:
          "url('https://images.unsplash.com/photo-1517248135467-4c7edcad34c4?auto=format&fit=crop&w=1600&q=40')",
        backgroundSize: "cover",
        backgroundPosition: "center",
      }}
    >
      {/* Dark overlay */}
      <div className="absolute inset-0 bg-naija-dark/90" />
      ...
      >
        <div className="grid lg:grid-cols-2 gap-14 lg:gap-20 items-start">
          ...
          <div className="flex flex-col gap-6">
            <div
              className="res-animate flex items-center gap-2"
              style={{
                opacity: 0,
            ...
            >
              <span className="w-6 h-[2px] bg-naija-red" />
            ...
            <p
              className="res-animate text-white/55 font-outfit text-base leading-relaxed max-w-sm"
              style={{
                opacity: 0,
                transform: "translateY(20px)",
                transition: "opacity 0.6s ease, transform 0.6s ease",
            ...
            >
              Book a table and enjoy an unforgettable Nigerian dining experience
              crafted for every occasion.
            </p>

            {/* Info Items */}
            <div
              className="res-animate grid gap-5"
              style={{
                opacity: 0,
                transform: "translateY(20px)",
                transition: "opacity 0.6s ease, transform 0.6s ease",
              ...
              {INFO_ITEMS.map((item) => {
                ...
                return (
                  <div key={item.title} className="flex items-start gap-4">
                    <div
                      className={`shrink-0 w-10 h-10 ${item.iconBg} rounded-xl flex items-center justify-center`}
                    >
                      <Icon size={18} className={item.iconColor} />
                    </div>
                    <div>
                      <p className="font-syne font-700 text-white text-sm mb-1">
                        {item.title}
                      </p>
                      {item.lines.map((line) => (
                        <p
                          key={line}
                          className="text-white/50 text-xs font-outfit leading-relaxed"
                        >
                          {line}
                        </p>
                      ))}
            ...
            {/* Social links */}
            <div
              className="res-animate flex flex-col gap-3"
              style={{
                opacity: 0,
                transform: "translateY(20px)",
                transition: "opacity 0.6s ease, transform 0.6s ease",
            ...
            <div className="bg-naija-card border border-naija-border rounded-3xl p-7 sm:p-8">
              {submitted ? (
                <div className="flex flex-col items-center justify-center gap-4 py-10 text-center">
                  ...
                  <div>
                    <h3 className="font-syne font-700 text-white text-xl mb-1">
                      Booking Confirmed!
                    </h3>
                ...
                <>
                  <h3 className="font-syne font-700 text-white text-xl mb-6">
                    Book Your Table
                  </h3>
                  ...
                  <form onSubmit={handleSubmit} className="flex flex-col gap-4">
                    ...
                    <div className="flex flex-col gap-1.5">
                      <label className="text-white/60 text-xs font-outfit font-500">
                        Full Name
                      </label>
                      <input
                        type="text"
