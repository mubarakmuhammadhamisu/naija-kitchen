...
export default function Footer() {
  return (
    <footer id="contact" className="bg-naija-card border-t border-naija-border">
      {/* Main footer */}
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-16">
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-5 gap-10">
          ...
          <div className="lg:col-span-2 flex flex-col gap-5">
            {/* Logo */}
            <Link href="#home" className="flex items-center gap-2 group self-start">
            ...
            <p className="text-white/45 font-outfit text-sm leading-relaxed max-w-xs">
              Authentic Nigerian cuisine crafted with love, tradition and the
              finest Nigerian ingredients. From smoky suya to rich jollof —
              every dish tells a story.
            </p>

            {/* Newsletter */}
            <div className="flex flex-col gap-2 mt-2">
              <p className="font-syne font-700 text-white text-sm">
                Get Weekly Specials
              </p>
          ...
          {Object.entries(FOOTER_LINKS).map(([title, links]) => (
            <div key={title} className="flex flex-col gap-4">
              <h4 className="font-syne font-700 text-white text-sm">{title}</h4>
      ...
      <div className="border-t border-naija-border">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-5 flex flex-col sm:flex-row items-center justify-between gap-3">
          <p className="text-white/35 text-xs font-outfit text-center sm:text-left">
            © 2025 Naija Kitchen. All rights reserved.
          </p>
          <p className="text-white/35 text-xs font-outfit flex items-center gap-1.5">
            Made with <Heart size={11} className="text-naija-red fill-naija-red" /> in Lagos, Nigeria
          </p>
        </div>
      </div>
    </footer>
